    ...
    for index in range(len(sorted_items)):
        if sorted_items[index] in train_pos_item:
            ...
            if sorted_items[index] in filter_item and \
                ...
                # Rank starts from 0
                dcg += 1 / math.log2(train_hit_num + valid_num + 1)
...
def load_data():
    ...
    train_neg = np.load(os.path.join(data_dir, "train_neg.npy"))
...
def evaluate():
    ...
    # Get the root directory path and kg.txt path
    root_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    kg_path = os.path.join(root_dir, 'data', 'kg.txt')
    ...
    ctr_time = time.time() - start_time - init_time \
               - train_time
